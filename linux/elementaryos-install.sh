## That's the installation script for elementary OS.
echo "Installing additional repos"
## First enable the possibility to add repos.
...
apt-get -y install git gist
apt-get -y install netcdf-bin libnetcdf-dev
## apt-get -y install libhdf5 libhdf5-dev  ??? not found?
...
# echo "Installing python libs" ?? pip not working?
...
##apt-get -y install handbrake-gtk not found
apt-get -y install ffmpeg ffmpegthumbnailer ffmpegthumbs
##apt-get -y install vidcutter not found
